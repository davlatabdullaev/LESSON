...
func main() {
	...
	// wg.Add(1)
	// go Exercise14(filename, &wg, ch)

	// go func() {
	// 	count := 0
	// 	for range ch {
	// 		count++
	// 		fmt.Println("File has ", count, " lines ")
	// 	}
...
func Exercise15B(ch chan int, wg *sync.WaitGroup) {
	defer wg.Done()
	counter := 0
	for i := 0; i <= 100; i++ {
		counter += i
...
func Exercise14(filename string, wg *sync.WaitGroup, ch chan string) {
